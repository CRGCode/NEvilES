...
namespace NEvilES.DataStore
{
    ...
    public abstract class Cache<T>
    {
        ...
        readonly ConcurrentDictionary<T, Task> _valuesPendingTasks = new ConcurrentDictionary<T, Task>();
        protected virtual async Task<IEnumerable<T>> AddIfEmptyAsync(Index index, Func<T> valueFactory, Func<T, Task> whenAdded)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _valuesPendingTasks.TryAdd(value, task);
                    AddToIndex(value);
                }
                finally
                {
            ...
            AddToPostIndex(value);
            _valuesPendingTasks.TryRemove(value, out task);
            return new[] { value };
        }

        public class Index 
        {
            private readonly Cache<T> _lookup;
            ...
            {
                var values = _lookup.indexes.GetOrAdd(Key, _ => _loader().ToList());
            ...
            {
                Task task;
                _lookup._valuesPendingTasks.TryGetValue(value, out task);
                return task;
            }

            public Task<IEnumerable<T>> AddIfEmptyAsync(Func<T> valueFactory, Func<T, Task> ifNew = null)
            {
                return _lookup.AddIfEmptyAsync(this, valueFactory, ifNew ?? (x => Task.FromResult(0)));
            }
        }
    }
}