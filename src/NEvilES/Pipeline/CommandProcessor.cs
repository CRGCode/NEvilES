...
namespace NEvilES.Pipeline
{
        ...
        where TCommand : IMessage
    {
        ...
        public CommandResult Process(TCommand command)
        {
            ...
            var repo = (IRepository)_factory.Get(typeof(IRepository));

            if (command is ICommand)
            {
                var type = typeof(IHandleAggregateCommandMarker<>).MakeGenericType(commandType);
                var aggHandlers = _factory.GetAll(type).Cast<IAggregateHandlers>().ToList();
                if (aggHandlers.Any())
                {
                    ...
                    var deps = new object[] { command }.Concat(parameters.Skip(1).Select(x => _factory.Get(x.ParameterType))).ToArray();
                ...
                var commandHandlers = _factory.GetAll(commandProcessorType).Cast<object>().ToArray();
            ...
            else
            {
                ...
                var singleAggHandler = _factory.TryGet(type);
            ...
            var projectResults = ReadModelProjectorHelper.Project(commandResult, _factory, _commandContext);
            return _commandContext.Result.Add(projectResults);
        }
    }
